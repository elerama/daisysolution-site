...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const pageUrl = `${siteUrl}/funzionalita/${slug}`
...
export default async function ModulePage({ params }: PageProps) {
    ...
    const breadcrumbs = [
        ...
        { name: moduleData.name, url: `/funzionalita/${slug}` },
    ]
                    ...
                    <Container className="py-24" id="integrazioni">
                        ...
                        <div className="mt-16 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 max-w-4xl mx-auto">
                            {moduleData.integrations.map((integration, idx) => (
                                <Link
                                    key={idx}
                                    href={`/funzionalita/${integration.slug}`}
                                ...
                                >
                                    <HeroIcons.LinkIcon className="h-10 w-10 text-brand-primary mb-3 group-hover:scale-110 transition-transform" />
