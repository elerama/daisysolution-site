...
/**
 * Client Component: Moduli Grid with Category Filters
 * Provides interactive filtering by category
 */
...
export function ModuliGridWithFilters() {
  ...
  // Get tier 1 modules (first 12) with category
  const tier1Slugs = moduleSlugs.filter(slug => modules[slug].tier === 1)
  const allModuli = tier1Slugs.map(slug => {
    ...
    <>
      ...
      <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {filteredModuli.map((modulo) => (
          ...
          >
            <div className="text-5xl mb-6" role="img" aria-label={modulo.title}>{modulo.icon}</div>
